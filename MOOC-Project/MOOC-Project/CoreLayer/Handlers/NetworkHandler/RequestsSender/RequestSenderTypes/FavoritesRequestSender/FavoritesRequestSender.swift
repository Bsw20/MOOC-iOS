...
class FavoritesRequestSender: IRequestSender {
    ...
    where Parser: IParser {
        ...
        guard let urlRequest = config.request.getUrlRequest(
            ...
            completonHandler(.failure(.badUrl(message: "INCORRECT URL - favorites courses REQUEST")))
            return
        }
        ...
        let task = session.dataTask(with: urlRequest) { (data: Data?, response: URLResponse?, error) in
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case 200:
                    Logger.logNetWork(description: "RESPONSE-CODE: 200 favorites courses received SUCCESSFULLY", logType: .success)
