...
class MainInfoRequestSender: IRequestSender {
    ...
    where Parser: IParser {
        ...
        guard let urlRequest = config.request.getUrlRequest(
            ...
            completonHandler(.failure(.badUrl(message: "INCORRECT URL - Main info REQUEST")))
            return
        }
        ...
        let task = session.dataTask(with: urlRequest) { (data: Data?, response: URLResponse?, error) in
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case 200:
                    Logger.logNetWork(description: "RESPONSE-CODE: 200 main info RECEIVED SUCCESSFULLY", logType: .success)
                    completonHandler(.success(parsedModel))
                    
                default:
                    Logger.logNetWork(description: "UNDEFINED BEHAVIOR - main info RESPONSE", logType: .error)
