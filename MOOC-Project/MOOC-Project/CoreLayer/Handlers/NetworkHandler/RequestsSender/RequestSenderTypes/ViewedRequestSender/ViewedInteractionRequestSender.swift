...
class ViewedIntercationRequestSender: IRequestSender {
    ...
    where Parser: IParser {
        ...
        guard let urlRequest = config.request.getUrlRequest(
            ...
            completonHandler(.failure(.badUrl(message: "INCORRECT URL - viewed interaction REQUEST")))
            return
        }
        ...
        let task = session.dataTask(with: urlRequest) { (data: Data?, response: URLResponse?, error) in
            ...
            if let response = response as? HTTPURLResponse {
                ...
                case 200:
                    Logger.logNetWork(description: "RESPONSE-CODE: 200 course is deleted from viewed SUCCESSFULLY", logType: .success)
                ...
                case 201:
                    Logger.logNetWork(description: "response-code: 201 course is added to viewed", logType: .success)
                    completonHandler(.success(parsedModel))
                case 404:
                ...
                default:
                    Logger.logNetWork(description: "UNDEFINED BEHAVIOR - VIEWED RESPONSE \(response.statusCode)", logType: .error)
